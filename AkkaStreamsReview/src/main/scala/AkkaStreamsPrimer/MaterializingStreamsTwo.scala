...
object MaterializingStreamsTwo extends App {

  implicit val system = ActorSystem("MaterializingStreams")
  implicit val materializer = ActorMaterializer()
  import system.dispatcher

  //Running the graph is materializing the graph and the result is the materialized value
  //A component can materialize multiple times
  //A materialized value can be ANYTHING and it mgith have to related to the elements in the stream
...
//  val g7 = wordCountFlow.runWith(sentenceSource, Sink.head)._2
  ...
  val sSource = Source(List("First phrase", "Second phrase", "Third phrase"))
  val iSource = Source(1 to 10)
  val sSink = Sink.last
  val res = sSource.toMat(Sink.last)(Keep.right).run()
  res.onComplete{
    case Success(value) => println("Conpleted: " + value)
    case Failure(t) => println(t.getMessage)
  }

  Source( 1 to 10).runWith(Sink.last)

  val wordCountFlow = Flow[String].fold[Int](0)((cWord,nWord)=>cWord + nWord.split(" ").length)
  val wordCountSink = Sink.fold[Int, String](0)((cSentence,nSentence) => cSentence + nSentence.split(" ").length)
  val wCount = sSource.via(wordCountFlow).toMat(Sink.head)(Keep.right).run()

  val count = sSource.runWith(wordCountSink)
  count.onComplete{
